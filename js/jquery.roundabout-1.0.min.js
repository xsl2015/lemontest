jQuery.extend({
	roundabout_shape: {
		...
		lazySusan: function(r, a, t) {
			return {
				x: Math.sin(r + a),
				y: (Math.sin(r + 3 * Math.PI / 2 + a) / 3) * t,
				z: (Math.cos(r + a) + 1) / 2,
				scale: (Math.sin(r + Math.PI / 2 + a) / 2)+0.3
			}
		}
	}
});
//scale: (Math.sin(r + Math.PI / 2 + a) / 2);
jQuery.fn.roundabout = function() {
	...
	options = {
		...
		minOpacity: (typeof options.minOpacity == 'undefined') ? 1 : parseFloat(options.minOpacity, 2),
		maxOpacity: (typeof options.maxOpacity == 'undefined') ? 1.00 : parseFloat(options.maxOpacity, 2),
		minScale: (typeof options.minScale == 'undefined') ? 0.50 : parseFloat(options.minScale, 2),
...
jQuery.fn.roundabout_updateChildPositions = function() {
	this.each(function(i) {
		...
		var data = {
			...
			stage: {
				width: Math.floor(ref.width() * 0.8),
				height: Math.floor(ref.height() * 0.8)
			},
		...
		//alert(data.midStage.width);
	//	alert(data.stage.width+"|"+data.midStage.width);
		data.midStage = {
